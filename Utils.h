...
Real read_OFF(std::string filename, std::vector<Vertex> &vertices, std::vector<Triangle> &faces, glm::vec3 &min);

std::string hashvertex(const glm::vec3 &vertex);
std::string hashedge(const glm::vec3 &v1, const glm::vec3 &v2);

void barycentric(glm::vec3 p, glm::vec3 a, glm::vec3 b, glm::vec3 c, Real &u, Real &v, Real &w);
